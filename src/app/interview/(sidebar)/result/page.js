...
function ClipVideoPlayer({ clip, onClipEnded }) {
  ...
  const dur = isRealClip
    ? videoRef.current?.duration || clipDuration || 6
    : clipDuration;
    ...
    <div className="relative w-full aspect-video bg-slate-950 rounded-xl overflow-hidden group">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 px-4 pb-3 pt-8 z-10">
        <div className="w-full h-1.5 bg-white/20 rounded-full mb-2">
          <div
            className="h-full bg-main rounded-full transition-[width] duration-75"
...
function InterviewPaceChart({ questions, averageWpm }) {
  ...
  const avgLineY =
    maxWpm > 0 ? ((maxWpm - (averageWpm || 0)) / maxWpm) * chartHeight : 0;
      ...
      <div className="relative" style={{ height: `${chartHeight}px` }}>
        ...
        <div className="relative ml-8 mr-2 h-full">
          ...
          <div
            ...
            style={{
              top: `${Math.max(0, idealTop)}px`,
              height: `${Math.max(4, idealBottom - idealTop)}px`,
            }}
          >
          ...
          >
            <span className="absolute -top-2.5 right-0 text-[8px] font-bold text-slate-400">
              avg
            </span>
          </div>
          ...
          <div className="absolute inset-0 flex items-end justify-around pb-1">
            {questions.map((q, i) => {
              const wpm = q.wpm || 0;
              const barHeight =
                maxWpm > 0 ? (wpm / maxWpm) * (chartHeight - 20) : 0;
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-10 md:w-14 rounded-t-md transition-all duration-500 cursor-pointer relative focus:outline-none ${getBarColor(
                      wpm,
            ...
            <div className="text-center">
              <span
                className={`block text-lg font-black ${
                  (questions[selectedQuestion].wpm || 0) > 150
                    ? "text-red-500"
                    : (questions[selectedQuestion].wpm || 0) >= 130
                      ? "text-orange-500"
                      : (questions[selectedQuestion].wpm || 0) >= 100
                        ? "text-green-600"
                        : (questions[selectedQuestion].wpm || 0) > 0
                          ? "text-blue-500"
                          : "text-slate-400"
                }`}
              >
...
export default function InterviewResultPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!raw) {
          setError(
            "No interview results found. Please complete an interview session first.",
          );
          setLoading(false);
          return;
        }
        ...
        try {
          ...
          if (storedClips.length > 0) {
            ...
            const realClips = storedClips.map((storedClip, i) => {
              ...
              const matchingEvent =
                events.find((e) => e.id === storedClip.id) || {};
              return {
          ...
          console.warn(
            "Failed to load clips from IndexedDB, falling back to full video:",
            e,
          );
        }
  ...
  const evaluateData =
    resultData?.evaluate_response?.data || resultData?.evaluate_response || {};
  ...
  const deliveryScore = Math.round(
    avgDeliveryFocus * 0.5 + avgDeliveryPace * 0.5,
  );
    ...
    async function postSession() {
      try {
        ...
        const interviewDetails = qaAnalysis.map((qa, i) => {
          ...
          return {
            question_number:
              qa.question_number || qData.question_number || i + 1,
            ...
            relevancy_score:
              qa.scores?.relevancy_score ?? qa.relevancy_score ?? 0,
            star_structure_score:
              qa.scores?.star_structure_score ?? qa.star_structure_score ?? 0,
            overall_answer_score:
              qa.scores?.overall_answer_score ?? qa.overall_answer_score ?? 0,
            strengths: qa.feedback?.strengths || qa.strengths || "",
            weaknesses: qa.feedback?.weaknesses || qa.weaknesses || "",
            recommended_answer:
              qa.feedback?.recommended_answer_improvement ||
              qa.recommended_answer ||
              "",
          };
        });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border-bold p-5 flex flex-col items-center">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 text-xs font-bold">
            <div className="text-center">
              <span className="block text-base font-black text-main">
                {aiContentScore}
              </span>
              <span className="text-[10px] text-slate-400">Content (65%)</span>
            </div>
            <div className="text-center">
              <span className="block text-base font-black text-blue-500">
                {deliveryScore}
              </span>
        ...
        <div className="bg-white border-bold p-5 flex flex-col items-center justify-center gap-2">
          ...
          <span className="text-2xl font-black text-slate-800">
            {metricsSummary.total_filler_words ??
              perQuestionData.reduce(
                (s, q) => s + (q.filler_words_count || 0),
                0,
              )}
          </span>
        ...
        <div className="bg-white border-bold p-5 flex flex-col items-center justify-center gap-2">
          ...
          <span className="text-2xl font-black text-slate-800">
            {metricsSummary.total_distract_duration ??
              perQuestionData.reduce(
                (s, q) => s + (q.distract_duration_seconds || 0),
                0,
              )}
      ...
      <div className="bg-white border-bold p-5">
                  ...
                  <div className="lg:w-[420px] shrink-0 flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() =>
                            setActiveClipIndex((p) => Math.max(-1, p - 1))
                          }
                        ...
                        <button
                          onClick={() =>
                            setActiveClipIndex((p) =>
                              Math.min(clips.length - 1, p + 1),
                            )
                          }
                    ...
                    <ClipVideoPlayer
                      clip={
                        activeClipIndex >= 0 ? clips[activeClipIndex] : null
                      }
                      onClipEnded={() => {
                ...
                <div className="flex-1 flex flex-col gap-6 min-w-0">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {perQuestionData.map((q, i) => {
                      ...
                      const maxDistract = Math.max(
                        ...perQuestionData.map(
                          (d) => d.distract_duration_seconds || 0,
                        ),
                        1,
                      );
                      const barColor =
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span
                              className={`text-xs font-black ${secs === 0 ? "text-green-600" : "text-slate-700"}`}
                            >
                              {secs === 0
                                ? "Perfect Focus!"
                                : `${secs}s distracted`}
                          ...
                          <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${maxDistract > 0 ? (secs / maxDistract) * 100 : 0}%`,
                              }}
                            />
                          </div>
                        </div>
                            ...
                            <div className="pb-5">
                              ...
                              <p className="text-xs text-slate-500 leading-relaxed">
                                {secs === 0
                                  ...
                                  : secs <= 3
                                    ? "Minor distraction — you stayed mostly focused."
                                    : secs <= 8
                                      ? "Moderate distraction — your focus slipped a few times."
        ...
        {activeTab === "filler" &&
          (() => {
            ...
            const hasAnyAnalysis = perQuestionData.some(
              (q) => q.speech_analysis,
            );
                ...
                {!hasAnyAnalysis && (
                  <div className="flex flex-col items-center gap-3 py-12">
                    ...
                    <span className="text-xs text-slate-300">
                      Filler word analysis requires microphone access during
                      your session.
                    </span>
                  </div>
                )}
              ...
              <div>
                ...
                {hasAnyAnalysis && !hasWordiness && (
                  <div className="flex flex-col items-center gap-3 py-12 mt-4">
                    ...
                    <span className="text-xs text-slate-400">
                      Your speech was concise and efficient — no redundant
                      phrases found.
                    </span>
                  </div>
                )}
        ...
        {activeTab === "ai_review" && (
          <div>
            ...
            {qaAnalysis.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-8">
                No AI review data available. Complete a full session to get AI
                feedback.
              </p>
            ) : (
              <AIReviewCarousel
